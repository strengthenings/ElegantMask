...
class Generator(nn.Module):#生成器
    def __init__(self, enc_dim=64, enc_layers=5, enc_norm_fn='batchnorm', enc_acti_fn='lrelu',
        ...
        layers = []
        n_in = 3
        for i in range(enc_layers):#构建了一个包含多个卷积层和转置卷积层的编码器
            #编码器将输入图像逐层下采样，直到特征图尺寸为4*4
        ...
        for i in range(dec_layers):#解码器层数为5,解码器则将输入噪声和属性向量级联，然后逐层上采样，直到生成的图像尺寸与原始输入相同。
            if i < dec_layers - 1:#不是最后一层
    ...
    def decode1(self, zs, zs1, m_z, r_z):#修改地方1，不清楚decode1函数是什么意思
        ms = []
        z1, _  = torch.split(zs[-1], 512-64, dim=1)
    ...
    def forward(self, x, a=None, b=None, c=None, mode='enc'):
        if mode == 'enc':
        ...
        if mode == 'dec':
            ...
            return z
        if mode =='dec1':
            z = self.decode1(x, a, b, c)
            return z
        raise Exception('Unrecognized mode: ' + mode)

class Discriminators(nn.Module):#鉴别器
...
class AttGAN():
    ...
    def trainG(self, img_a, img_b, att_a, att_b):
        ...
        img_recon_b  = self.G(zs_b, gen2_b, mode='dec')
        img_recon_a = self.G(zs_a, gen2_a, mode='dec')
        ...
        att_a_f = att_a.clone()
        att_a_f[:, 0] = att_b[:, 0]
        gc_loss = F.binary_cross_entropy_with_logits(dc_a_fake, att_b_1) + F.binary_cross_entropy_with_logits(dc_b_fake, att_a_1)
        gc_loss8 = F.binary_cross_entropy_with_logits(dc8_a_fake, att_a_f) + F.binary_cross_entropy_with_logits(dc8_b_fake, att_b_f)
    ...
    def trainD(self, img_a, img_b, att_a, att_b):
        ...
        if self.mode == 'wgan':
            wd = d_real_b.mean() - d_fake_a.mean() + d_real_a.mean() - d_fake_b.mean()
            df_loss = -wd
