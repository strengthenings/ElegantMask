    ...
    for img_c, att_c in progressbar(train_dataloader):
        ...
        if (it+1) % 500 == 0:
            ...
            with torch.no_grad():
                ...
                img_recon_b  = attgan.G(zs_b, gen2_b+gen2_a, mode='dec')
                img_recon_a = attgan.G(zs_a, gen2_b+gen2_a, mode='dec')
                
        
                img_fake_a = attgan.G(zs_a, zs_b, gen2_a, gen2_b, mode='dec1')
                img_fake_b = attgan.G(zs_b, zs_a, gen2_b, gen2_a, mode='dec1')
                img_fake_a = img_a*(1-mb128) + img_fake_a*mb128
                img_fake_b = img_b*(1-mb128) + img_fake_b*mb128
                ma =mb128.repeat(1, 3,  1, 1)
                viz_images = torch.stack([img_a,img_b,img_fake_a, img_fake_b, ma], dim=1)
